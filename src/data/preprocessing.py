...
def get_sentiment(text):
    ...
    threshold = config["preprocessing"]["sentiment_threshold"]

    return "negative" if score <= threshold else "non-negative"
...
def add_company_name(df, raw_data_path):
    ...
    company_lookup = (
        df_original[df_original["inbound"] == False][
            ["tweet_id", "author_id"]
        ]
        .rename(columns={"author_id": "company_name"})
    )
...
def run_pipeline(merged_path, raw_data_path, output_dir):
    ...
    raw_features = merged["customer_text"].apply(
    ...
    merged["customer_text_clean"] = (
        merged["customer_text"]
        .apply(clean_text)
        .fillna("")
        .str.strip()
    )

    merged["company_text_clean"] = (
        merged["company_text"]
        .apply(clean_text)
        .fillna("")
        .str.strip()
    ...
    merged["response_time_capped"] = (
        merged["response_time_mins"]
        .clip(
            upper=config["preprocessing"]["response_time_cap"]
        )
    )

    merged["response_time_log"] = np.log1p(
        merged["response_time_capped"]
    )

    merged = add_company_name(
        merged,
        raw_data_path
    )
